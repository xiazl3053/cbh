...
@implementation NewListRecordDB
...
- (void)createTables {
    //设置表名
    _tableName=@"NewListRecord";
    NSString *className=@"NewListRecordDB";
    //检测是否需要删表
...
-(BOOL)isExistNlm:(NewListModel *)nlm{
    ...
    //sql插入语句
    NSString *string=@"";
    //articleId text,picsId text,specialId text,videoId text,adId text
    switch (type) {
        case 2://专题
            string=[NSString stringWithFormat:@"select * from %@ where specialId='%@' order by id desc;",_tableName,nlm.specialId];
            break;
        case 3://图集
            string=[NSString stringWithFormat:@"select * from %@ where picsId='%@' order by id desc;",_tableName,nlm.picsId];
            break;
        case 4://视频
            string=[NSString stringWithFormat:@"select * from %@ where videoId='%@' order by id desc;",_tableName,nlm.videoId];
            break;
        case 5://推广
            string=[NSString stringWithFormat:@"select * from %@ where adId='%@' order by id desc;",_tableName,nlm.adId];
            break;
        default:
            string=[NSString stringWithFormat:@"select * from %@ where articleId='%@' order by id desc;",_tableName,nlm.articleId];
            break;
    }
