...
-(void)insertNlm:(NewListModel *)nlm timeType:(NSString *)timeType{
    ...
    //sql插入语句
    NSString *string=[NSString stringWithFormat:@"insert into %@(programId,type,articleId,picsId,specialId,videoId,adId,picUrls,title,desc,followNum,adUrl,videoUrl,order1,addtime,timeType) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);",_tableName];
    const char *sql = [string UTF8String];
    sqlite3_stmt *stmt;
    ...
    // 说明语句没有语法问题
    if (result == SQLITE_OK) {
        const char *programId1=[nlm.programId UTF8String];
        ...
        const char *timeType1=[timeType UTF8String];
        ...
        sqlite3_bind_text(stmt, 16, timeType1, -1, NULL);
...
-(void)deleteNdmWithTimeType:(NSString *)timeType{
    //sql插入语句
    NSString *string=[NSString stringWithFormat:@"delete from %@ where (timeType='%@');",_tableName,timeType];
    const char *sql = [string UTF8String];
    sqlite3_stmt *stmt;
    ...
    if (result == SQLITE_OK) {
        // 执行sql语句
        if (sqlite3_step(stmt)==SQLITE_DONE) {
            NSLog(@"推送列表删除timeType:%@数据成功！",timeType);
        } else {
            NSLog(@"推送列表删除数据失败！");
        }
    } else {
        NSLog(@"推送列表删除数据的SQL语句语法有问题");
    }
    sqlite3_finalize(stmt);
}
...
-(NSMutableArray *)getNewListWithTimeType:(NSString *)timeType{
    ...
    NSString *string=[NSString stringWithFormat:@"select * from %@ where timeType='%@' order by id desc;",_tableName,timeType];
    const char *sql = [string UTF8String];
