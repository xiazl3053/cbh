...
- (void)createTables {
    //设置表名
    _tableName=@"PicsList2";
    NSString *className=@"PicsListDB2";
    //检测是否需要删表
    [[CommonOperation getId] checkTableUpdateWithTableName:_tableName className:className db:_db];
    
    NSString *string=[NSString stringWithFormat:@"create table if not exists %@(id integer primary key autoincrement, hostPicsId text,picsId text,type text,title text,followNum text,picUrls text,order1 text,addtime text,programId text);",_tableName];
...
-(void)insertPlm:(PicsListModel *)plm hostPicsId:(NSString *)hostPicsId{
    //sql插入语句
    NSString *string=[NSString stringWithFormat:@"insert into %@(hostPicsId,picsId,type,title,followNum,picUrls,order1,addtime,programId) values(?,?,?,?,?,?,?,?,?);",_tableName];
    const char *sql = [string UTF8String];
    sqlite3_stmt *stmt;
    ...
    if (result == SQLITE_OK) {
        ...
        sqlite3_bind_text(stmt, 9, programId, -1, NULL);
...
-(void)deletePlmWithHostPicsId:(NSString *)hostPicsId{
    ...
    if (result == SQLITE_OK) {
        // 执行sql语句
        if (sqlite3_step(stmt)==SQLITE_DONE) {
            NSLog(@"图集列表删除hostPicsId:%@数据成功！",hostPicsId);
        } else {
            NSLog(@"图集列表删除数据失败！");
        }
    } else {
...
-(NSMutableArray *)getPlmsWithHostPicsId:(NSString *)hostPicsId{
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW){
            ...
            NSString *programId=[[NSString alloc] initWithCString:s9 encoding:NSUTF8StringEncoding];
