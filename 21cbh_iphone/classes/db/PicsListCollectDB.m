...
@implementation PicsListCollectDB
...
- (void)createTables {
    //设置表名
    _tableName=@"PicsListCollect";
    NSString *className=@"PicsListCollectDB";
    //检测是否需要删表
...
-(void)insertPlm:(PicsListModel *)plm{
    ...
    // 说明语句没有语法问题
    if (result == SQLITE_OK) {
        const char *programId=[plm.programId UTF8String];
...
-(void)deletePlm:(PicsListModel *)plm{
    //sql插入语句
    NSString *string=[NSString stringWithFormat:@"delete from %@ where (programId='%@' and picsId='%@');",_tableName,plm.programId,plm.picsId];
    const char *sql = [string UTF8String];
    sqlite3_stmt *stmt;
    ...
    if (result == SQLITE_OK) {
        // 执行sql语句
        if (sqlite3_step(stmt)==SQLITE_DONE) {
            NSLog(@"图集收藏列表删除picsId:%@数据成功！",plm.picsId);
        } else {
            NSLog(@"图集收藏列表删除数据失败！");
        }
    } else {
        NSLog(@"图集收藏列表删除数据的SQL语句语法有问题");
    }
    sqlite3_finalize(stmt);
}
...
-(void)deletePlm2:(PicsListModel *)plm{
    //sql插入语句
    NSString *string=[NSString stringWithFormat:@"delete from %@ where (title='%@');",_tableName,plm.title];
    const char *sql = [string UTF8String];
    sqlite3_stmt *stmt;
...
-(BOOL)isExistPlmWithPicsId:(NSString *)picsId programId:(NSString *)programId{
    bool b=NO;
    NSString *string=[NSString stringWithFormat:@"select * from %@ where picsId='%@' and programId='%@' order by id desc;",_tableName,picsId,programId];
    const char *sql = [string UTF8String];
...
-(BOOL)isExistPlmWithTitle:(NSString *)title{
    bool b=NO;
    NSString *string=[NSString stringWithFormat:@"select * from %@ where title='%@' order by id desc;",_tableName,title];
    const char *sql = [string UTF8String];
