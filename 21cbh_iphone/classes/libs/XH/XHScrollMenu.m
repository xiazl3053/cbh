...
- (UIImageView *)getShadowView:(BOOL)isLeft {
    UIImageView *shadowImageView = [[UIImageView alloc] initWithFrame:CGRectMake(10, (self.frame.size.height-25)*0.5f, 1, 23)];
    shadowImageView.backgroundColor=UIColorFromRGB(0xa0a0a0);
...
- (void)setup {
    ...
    _rightShadowView = [self getShadowView:YES];
    ...
    _managerMenusButton.backgroundColor = [UIColor clearColor];
//  [_managerMenusButton setImage:[UIImage imageNamed:@"settings"] forState:UIControlStateNormal];
//  [_managerMenusButton setImage:[UIImage imageNamed:@"settings_selected"] forState:UIControlStateHighlighted];
    ...
    rightShadowViewFrame.origin = CGPointMake(CGRectGetMinX(_managerMenusButton.frame) - CGRectGetWidth(rightShadowViewFrame)-6, (self.frame.size.height-rightShadowViewFrame.size.height)*0.5f);
    _rightShadowView.frame = rightShadowViewFrame;
    
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_leftShadowView.frame), 0, CGRectGetWidth(self.bounds) - CGRectGetWidth(rightShadowViewFrame) * 2 - CGRectGetWidth(_managerMenusButton.frame)-10, CGRectGetHeight(self.bounds))];
...
- (XHMenuButton *)menuButtonAtIndex:(NSUInteger)index {
    if (index>self.menuButtons.count-1) {
        return [self.menuButtons firstObject];
    }
    
    return [self.menuButtons objectAtIndex:index];
}
...
- (void)reloadData {
    ...
    for (XHMenu *menu in self.menus) {
        ...
        menuButtonFrame.size.height=self.frame.size.height;
