...
@interface ZXPhotoBrowser () <MJPhotoViewDelegate>
{
    UIView *_top;//标题栏
...
@property(strong,nonatomic)NoticeOperation *no;//提示view操作类
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    self.ndv.currentIndex=_currentPhotoIndex;
...
-(void)initTop{
    ...
    UIView *topLine=[[UIView alloc] initWithFrame:CGRectMake(0,_top.frame.size.height-0.5f, _top.frame.size.width,0.5f)];
    self.view.backgroundColor=k000000;
    topLine.backgroundColor=K808080;
    [_top addSubview:topLine];
    //添加蒙版
//    UIView *top2=[[UIView alloc] initWithFrame:_top.frame];
//    top2.backgroundColor=[UIColor whiteColor];
//    top2.alpha=0.1f;
//    [self.view addSubview:top2];
    if (self.backView) {
        self.backView.backgroundColor=[UIColor clearColor];
        //添加蒙版
        UIView *backView2=[[UIView alloc] initWithFrame:self.backView.frame];
        backView2.backgroundColor=k000000;
        backView2.alpha=0.1f;
...
- (void)createToolbar
{
    CGFloat barHeight =104;
    CGFloat barY = self.view.frame.size.height - barHeight;
    _toolbar = [[ZXPhotoToolbar alloc] init];
    _toolbar.frame = CGRectMake(0, barY, self.view.frame.size.width, barHeight);
    _toolbar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    _toolbar.backgroundColor=[UIColor clearColor];
    _toolbar.photos = _photos;
    _toolbar.descs=_ndv.ndm.descs;
