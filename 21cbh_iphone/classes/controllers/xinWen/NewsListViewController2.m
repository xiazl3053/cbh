...
NSString *const MJTableViewCellIdentifier10 = @"table";

@interface NewsListViewController2 (){
    ...
    NSString *_page;
    NSString *_pageNum;
    NSString *_newListId;//最新的新闻列表id,让服务器判断是否传回最新数据
    NSString *_order;
    NSString *_isNew;
}
...
@implementation NewsListViewController2
...
-(void)initViews{
    ...
    // 1.注册
    [_table registerClass:[UITableViewCell class] forCellReuseIdentifier:MJTableViewCellIdentifier10];
    // 3.集成刷新控件
    // 下拉刷新
    [self addHeader];
    //上拉加载更多
...
-(void)getNewsListWithisUp:(BOOL)isUp{
    ...
    hmgr.hh.nlv2=self;
    if (isUp) {//下拉刷新
        if (!_nlms||_nlms.count<1) {//第一次刷新
            [hmgr newsList2WithProgramId:self.programId type:@"" id:_newListId order:_order addtime:@"0" page:_page pageNum:_pageNum isUp:isUp];
    ...
    NSString *newListId=[self getNewsListItemId:nlm];
    [hmgr newsList2WithProgramId:self.programId type:nlm.type id:newListId order:nlm.order addtime:nlm.addtime page:_page pageNum:_pageNum isUp:isUp];
}
    ...
    if (nlms&&nlms.count>0) {//有数据才执行下面的代码
        ...
        if (isUp) {//刷新全部替换数据
            ...
            if (data&&[data isKindOfClass:[NSDictionary class]]) {
                ...
                _isNew=[data objectForKey:@"isNew"];
                if (_isNew) {//数据是最新的,保存最新的新闻id
                    if ([_isNew isEqualToString:@"1"]) {
                        NewListModel *nlm=nil;
                        nlm=[_nlms objectAtIndex:0];
                        _newListId=[self getNewsListItemId:nlm];
                        _order=nlm.order;
                    }
                }
            }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [_table reloadData];
            if (isUp) {//推过来的新闻和本地数据库匹对,提示用户有多少条新闻没读
                dispatch_async(dispatch_get_global_queue(0, 0), ^{
                    
                    __block NSInteger pageCount=_nlms.count;
                    
                    [_dbQueue addOperationWithBlock:^{
                        for (NewListModel *nlm in _nlms) {
                            BOOL b=[_nlrDB isExistNlm:nlm];
                            if (b) {
                                pageCount--;
                            }
                        }
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (pageCount!=0) {
                                if ([_isNew isEqualToString:@"1"]){
                                    [[NoticeOperation getId] showRecommendViewWithFrame:CGRectMake((self.view.frame.size.width-312)*0.5f, KTopHeight+5, 312, 25) superView:_table msg:[NSString stringWithFormat:@"更新了%i条新资讯",pageCount]];
                                }else{
                                    [[NoticeOperation getId] showRecommendViewWithFrame:CGRectMake((self.view.frame.size.width-312)*0.5f, KTopHeight+5, 312, 25) superView:_table msg:[NSString stringWithFormat:@"为您推荐了%i条未读资讯",pageCount]];
                                }
...
#pragma mark 获取新闻列表item的id
-(NSString *)getNewsListItemId:(NewListModel *)nlm{
    //有数据刷新
    NSString *newListId=@"";
