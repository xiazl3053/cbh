...
@interface NewsDetailViewController2 (){
    ...
    __block UIWebView *_web;
    UIView *_loadView;
    UIView *_reLoadview;
    UIView *_bottom;
    UIView *_commentView;
    UIView *_chatNumView;//消息数view
    __block UIButton *_collectBtn;//带星星的收藏按钮
...
-(void)viewDidDisappear:(BOOL)animated{
    ...
    //添加文章查看时长统计
    [[Frontia getStatistics]eventEnd:@"voice_pv" eventLabel:[NSString stringWithFormat:@"%@:%@:%@",self.ndm.title,self.programId,self.articleId]];
...
#pragma mark - ------------PlayerTool代理方法------------
#pragma mark -曲目切换
-(void)PlayerTool2:(PlayerTool2 *)tool voiceChange:(VoiceListModel *)model{
    
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [self changeNews:[[PlayManager sharedPlayManager] getNowVoiceModel]];
...
-(void)initViews{
    ...
    UIWebView *web=[[UIWebView alloc] initWithFrame:CGRectMake(0, top.frame.origin.y+top.frame.size.height, self.view.frame.size.width, self.view.frame.size.height-top.frame.size.height-44-20-31)];
...
//    
//    
//    UIImage *img1=[UIImage imageNamed:@"newsDetail_forward"];
...
//    
//    
//    UIImage *img2=[UIImage imageNamed:@"newsDetail_collect"];
...
-(void)setCollectBtnImage{
    [self.dbQueue addOperationWithBlock:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (b) {
                [_collectBtn setImage:[UIImage imageNamed:@"PlayTool_Inside_Collect"] forState:UIControlStateNormal];
                [_collectBtn setImage:[UIImage imageNamed:@"PlayTool_Inside_Collect"] forState:UIControlStateHighlighted];
            }else{
                [_collectBtn setImage:[UIImage imageNamed:@"PlayTool_Inside_noCollect"] forState:UIControlStateNormal];
                [_collectBtn setImage:[UIImage imageNamed:@"PlayTool_Inside_noCollect"] forState:UIControlStateHighlighted];
            }
        });
...
-(void)collectBtn{
    ...
    [self.dbQueue addOperationWithBlock:^{
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (!self.ndm||self.ndm.programId.length<2||self.ndm.articleId.length<2) {
                return;
            }
...
-(void)initWeb{
    ...
    UIWebView *web=[[UIWebView alloc] initWithFrame:_web.frame];
    web.backgroundColor=[UIColor clearColor];
    web.scrollView.indicatorStyle=UIScrollViewIndicatorStyleBlack;
    web.delegate=self;
    web.opaque=NO;//背景不透明设置为NO
...
#pragma mark 切换加载新闻
-(void)changeNews:(VoiceListModel *)model{
    dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)loadData{
    ...
    _loadView=[[NoticeOperation getId] getLoadView:_web imageName:@"alert_load"];
    if (_b) {
...
-(void)getNewsDetailHandle:(NewsDetailModel *)ndm{
    ...
    if (ndm&&![ndm.articleId isEqualToString:[[PlayManager sharedPlayManager] getNowVoiceModel].articleId]) {//判断到当前显示的文章和正在播放的音频不一致,再请求一遍
        [self changeNews:[[PlayManager sharedPlayManager] getNowVoiceModel]];
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{//没加载成功出线重新加载view
        ...
        if (!ndm||ndm.body.length<10) {
            if (!_b) {
                [self initWeb];
                _reLoadview=[[NoticeOperation getId] getReLoadview:_web obj:self imageName:@"alert_load"];
            }
            return;
        }
        ...
        [[Frontia getStatistics]eventStart:@"voice_pv" eventLabel:[NSString stringWithFormat:@"%@:%@:%@",self.ndm.title,self.programId,self.articleId]];
...
-(void)initPlayer{
    ...
    PlayerTool2 *tool=[[PlayerTool2 alloc]initWithFrame:CGRectMake(0, self.view.height-75, 320, 75)];
    tool.delegate=self;
    UIButton *collect=(UIButton *)[tool viewWithTag:88];
    _collectBtn=collect;
    [self setCollectBtnImage];
    _tool=tool;
