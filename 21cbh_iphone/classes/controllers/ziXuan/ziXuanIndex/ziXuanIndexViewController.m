...
@interface ziXuanIndexViewController ()<UIAlertViewDelegate>{
...
@property (nonatomic,strong) NSTimer *mainTableViewTimer;

@end

static ziXuanIndexViewController *_zixuanList = nil;

@implementation ziXuanIndexViewController
...
+(ziXuanIndexViewController *)instance{
    @synchronized(self){
        if (_zixuanList==nil) {
            _zixuanList = [[ziXuanIndexViewController alloc] init];
        }
    }
    return _zixuanList;
}
...
-(void)show{
    ...
    if (_huShen) {
        ...
        CGFloat screenHeight = KScreenSize.height;
        screenHeight = screenHeight - 20 - 44 - 30 - 61 - 25;
        CGFloat mainHeight = _mainTableView.mainView.frame.size.height+_mainTableView.mainView.frame.origin.y;
        // 当没有自选股数据时候
        ...
        [UIView animateWithDuration:0.3 animations:^{
            _huShen.view.frame = CGRectMake(_mainTableView.frame.origin.x, h+tipHeight, self.view.frame.size.width, 25);
        } completion:^(BOOL finish){
        
        }];
        
        // 管理界面也得跟随高度变化
        _ziXuanManage.view.frame = CGRectMake(_ziXuanManage.view.frame.origin.x, 0, self.view.frame.size.width, h+52+tipHeight);
        [_ziXuanManage show];
    }
...
-(void)initParam{
    ...
    // 网络异常回调 在此请处理好网络异常事件
    __unsafe_unretained ziXuanIndexViewController *zx = self;
...
-(void)initDidView{
    ...
    if (!_mainTableView) {
        // 行情表格
        CGFloat h = self.view.frame.size.height-61.5;
        _mainTableView = [[mainTableView alloc] initWithController:self andFrame:CGRectMake(0,0,self.view.frame.size.width,h)];
        ...
        line = nil;
        // 回调标题点击事件
        __block __unsafe_unretained ziXuanIndexViewController *dp = self;
...
//                }
        ...
        // 创建自选管理视图
        _ziXuanManage = [[ziXuanManageViewController alloc] init];
        [self addChildViewController:_ziXuanManage];
        _ziXuanManage.Parent = self.Parent;
        h = self.view.frame.size.height-58;
        _ziXuanManage.view.frame = CGRectMake(-self.view.frame.size.width, 0, self.view.frame.size.width, h);
        [self.view addSubview:_ziXuanManage.view];
        
    }
    
    // 点击旋转按钮 回调块
    __unsafe_unretained ziXuanIndexViewController *_dp = self;
    ...
    // 放沪深股在底部
    if (self.isShowHuShenZhi) {
        if (!_huShen) {
            CGFloat h = _mainTableView.mainView.frame.size.height + _mainTableView.mainView.frame.origin.y+0.5;
            _huShen = [[huShenViewController alloc] initWithParent:self andFrame:CGRectMake(0, 0, self.view.frame.size.width, h)];
        }
    }
}
        ...
        [_queue addOperationWithBlock:^{
            ...
            if (_mainTableView && _mainTableView.frame.origin.x<=0) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (_leftData.count<=0) {
                        _ziXuanManage.valueData = _leftData;
...
-(void)clickAddButtonAction:(UIButton*)button{
    ...
    [_queue addOperationWithBlock:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.Parent.navigationController pushViewController:search animated:YES];
            _addView.backgroundColor = UIColorFromRGB(0xFFFFFF);
            search = nil;
        });
    }];
...
-(void)addMainTableViewTimer{
    self.mainTableViewTimer = [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(setMainTableViewTimer) userInfo:nil repeats:YES];
//[[NSRunLoop currentRunLoop] addTimer:_mainTableViewTimer forMode:NSRunLoopCommonModes];
...
-(void)delMainTableViewTimer{
    [_mainTableViewTimer setFireDate:[NSDate distantFuture]];
    [_mainTableViewTimer invalidate];
    _mainTableViewTimer = nil;
}
